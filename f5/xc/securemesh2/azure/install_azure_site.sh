...
cat <<EOF
    ...
    if action == install:-
    Usage(): bash $0 install <image_version> <ver-type> <region> <token>
    
    E.g.,
    * install  9.2024.40  single|multi  eastus  <jwt-token> - Installs the infrastructure in the selected region in the azure cloud
EOF
exit 1
...
cat <<EOF
    ...
    E.g.,
    * destroy - destroys the infrastructure in the selected region in the azure cloud
EOF
exit 1
else
cat <<EOF
...
CreateSMInfrastructure() {
    ...
    terraform init
    terraform plan -var "node_prefix=$project_prefix" -var "instances=$vm_nodes" -var "az_location=$region" -var "image_version=$image_version" -var "cluster_token=$token" -out $$_terraform.plan
